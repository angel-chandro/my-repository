...
#print(b)
"""
h = np.loadtxt("Satellite_Galaxy_Halpha_All_z_1.321_Perfiles.dat" , comments = '#')
ID_G = h[:,0]
print("okele1")
print(len(ID_G))
"""
"""
for i in range( 0 , len(ID_G) , 1):
   ID_G_i = int(ID_G[0])
   print(ID_G_i)
"""
"""
...
for j in range( 0 , b , 1 ):
       ...
       if u1 == p:
          #r = r + 1
          #print(r)
          file10 = open("/data5/UNITSIM/1Gpc_4096/fixedAmp_001/ROCKSTAR/tree/"+u)
          ...
          if u2 != "#" and u2 == " " and float(u3) == 0.4309:
                print(u3)
                ...
                outfile.write('%f %d %f %f %f %f %f %f %.8e %f %d \n' %(float(u1),u4,float(ar[17]),float(ar[18]),float(ar[19]),float(ar[20]),float(ar[21]),float(ar[22]),float(ar[10]),float(ar[11]),int(ar[30])))
